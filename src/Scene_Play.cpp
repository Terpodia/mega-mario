...
void Scene_Play::init(const std::string& levelPath) {
    ...
    while (fin >> type) {
        if (type == "Dec") {
            ...
            fin >> m_playerConfing.X >> m_playerConfing.Y >>
                m_playerConfing.CX >> m_playerConfing.CY >>
                m_playerConfing.SPEED >> m_playerConfing.MAXSPEED >>
                m_playerConfing.JUMP >> m_playerConfing.GRAVITY >>
                m_playerConfing.WEAPON;
            addPlayer();
        }
    }
}
...
void Scene_Play::sAnimation() {
    ...
    auto spriteScale = anim.animation.getSprite().getScale();
    anim.animation.getSprite().setScale(spriteScale.x * flip,
                                        spriteScale.y);
